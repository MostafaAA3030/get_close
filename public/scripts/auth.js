...
getId("register_form").addEventListener('submit', function(ev) {
  ev.preventDefault();
  registerForm();
});
...
/*
  validateName (
    {uname: getId("uname").value}, 
    {letter_pattern: /^[a-zA-Z]+$/}, 
    {id: "uname_demo"}
  )
    .then(function (suc_res) {
      showDemo(suc_res);  
    })
    .catch(function(unsuc_res) {
      showDemo(unsuc_res);
    })
...
//  alert(user_data);
  ...
  xhr.open("POST", "http://localhost:5000/users/register", true);
